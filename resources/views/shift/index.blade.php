...
@section('content')
    <div class="container-fluid p-4">
        <div class="mb-4">
            <h4 class="font-weight-bold mb-1" style="color: var(--text-main); font-size: 1.5rem;">Manage Shifts</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent p-0 small" style="margin-bottom: 0;">
                    ...
                    <li class="breadcrumb-item active text-muted" aria-current="page">Shift</li>
                </ol>
            </nav>
        </div>
        ...
        <div class="d-flex justify-content-end mb-3">
            <a href="javascript:void(0);" data-url="{{ route('admin.shift.create') }}" data-ajax-popup="true"
    ...
    <div class="data-table-card-container container-fluid" data-fetch-url="{{ route('admin.shift.getShiftData') }}">
            ...
            <div class="col-12">
                <div class="responsive-table-overflow-wrapper">
                    <table class="core-structured-dark-table">
                        <thead>
                            <tr>
                                <th data-sort="name" style="cursor: pointer;">{{ __('Shift') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="branch_id" style="cursor: pointer;">{{ __('Branch') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="start_at" style="cursor: pointer;">{{ __('Shift S.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="break_start_at" style="cursor: pointer;">{{ __('Break S.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="break_end_at" style="cursor: pointer;">{{ __('Break E.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="end_at" style="cursor: pointer;">{{ __('Shift E.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="total_hours" style="cursor: pointer;">{{ __('Total Hours') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="general_ot_start_at" style="cursor: pointer;">{{ __('General OT S.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="general_ot_end_at" style="cursor: pointer;">{{ __('General OT E.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="extra_ot_start_at" style="cursor: pointer;">{{ __('Extra OT S.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th data-sort="extra_ot_end_at" style="cursor: pointer;">{{ __('Extra OT E.T.') }} <i
                                        class="fa-solid fa-sort sort-indicator-caret-icon"></i></th>
                                <th class="aligned-right-column">{{ __('Action') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <!-- ডেটা জাভাস্ক্রিপ্ট ইঞ্জিন দিয়ে এখানে ডাইনামিকালি রেন্ডার হবে -->
                        </tbody>
                    </table>
                </div>
            </div>
